...
export default function App() {
  ...
  useEffect(() => {
    // Cargar fichas guardadas del localStorage
    const loadFichasGuardadas = () => {
      try {
        ...
        console.error('Error loading fichas guardadas:', error);
      }
    };
  ...
  const handleSaveFicha = () => {
    ...
    // Guardar ficha básica (estado: creada)
  ...
  const handleEntradaComplete = (entradaData: any) => {
    // Actualizar ficha con datos de entrada
    const fichasActualizadas = fichasGuardadas.map(ficha => {
      if (ficha.folio === fichaData.folio) {
        // Si ya tiene salida, marcar como completada
        const nuevoEstado = ficha.salidaData ? 'completada' : 'con-entrada';
    ...
    // Actualizar selectedFicha para que se refleje inmediatamente en DetalleFicha
    const fichaActualizada = fichasGuardadasFinal.find(f => f.folio === fichaData.folio);
    if (fichaActualizada) {
      setSelectedFicha(fichaActualizada);
    }
    
    // Mostrar mensaje apropiado
    if (fichaActualizada?.estado === 'completada') {
      toast.success('Ficha completada. Entrada registrada correctamente');
    } else {
      toast.success('Entrada registrada. Ahora puedes completar la salida');
    }
    
    setCurrentScreen('detalle-ficha');
  };

  const handleSalidaComplete = (salidaData: any) => {
    // Actualizar ficha con datos de salida
    const fichasActualizadas = fichasGuardadas.map(ficha => {
      if (ficha.folio === fichaData.folio) {
        // Si ya tiene entrada, marcar como completada
        const nuevoEstado = ficha.entradaData ? 'completada' : 'con-salida';
        return { ...ficha, estado: nuevoEstado as const, salidaData };
      }
      return ficha;
    });
    ...
    // Mostrar mensaje apropiado
    if (fichaActualizada?.estado === 'completada') {
      toast.success('Ficha completada. Salida registrada correctamente');
    } else {
      toast.success('Salida registrada. Ahora puedes completar la entrada');
    }
